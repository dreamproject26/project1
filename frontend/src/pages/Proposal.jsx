...
import { documents, goldTexture } from '@/data/mockData';
...
const Proposal = () => {
  ...
  const submit = (e) => {
    e.preventDefault();
    const store = JSON.parse(localStorage.getItem('nnv_proposals') || '[]');
    store.unshift({ ...form, id: Date.now(), created_at: new Date().toISOString(), status: 'New' });
    localStorage.setItem('nnv_proposals', JSON.stringify(store));
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-24 bg-primary text-primary-foreground overflow-hidden">
        ...
        <div className="relative container-executive">
          ...
          <h1 className="mt-6 font-display font-semibold text-4xl sm:text-5xl lg:text-6xl leading-[1.05] max-w-4xl">
            Request a proposal. Download the profile.
          </h1>
          <p className="mt-6 text-lg text-primary-foreground/75 max-w-2xl">
            Every submission is reviewed by the founding office. Responses are structured and typically returned within two business days.
          </p>
        </div>
      </section>
          ...
          <div className="lg:col-span-7">
            ...
            <form onSubmit={submit} className="mt-10 grid sm:grid-cols-2 gap-5">
              <div className="sm:col-span-1">
                ...
                <Input required value={form.full_name} onChange={(e) => set('full_name', e.target.value)} placeholder="Your name" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
                <Label>Company / Organisation</Label>
                <Input value={form.organization} onChange={(e) => set('organization', e.target.value)} placeholder="Company name" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
                <Label>Designation</Label>
                <Input value={form.designation} onChange={(e) => set('designation', e.target.value)} placeholder="Role" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
                <Label>Business Type</Label>
                <Input value={form.business_type} onChange={(e) => set('business_type', e.target.value)} placeholder="Sector / industry" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
                <Label>Email</Label>
                <Input required type="email" value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="you@company.com" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
                <Label>Phone / WhatsApp</Label>
                <Input value={form.phone} onChange={(e) => set('phone', e.target.value)} placeholder="+880 …" className="mt-2" />
              </div>
              <div className="sm:col-span-1">
              ...
              <div className="sm:col-span-1">
                ...
                <div className="mt-2 flex items-center gap-2 border border-input rounded-sm h-10 px-3 text-sm text-muted-foreground cursor-pointer hover:border-accent transition-colors">
                  <Upload className="h-4 w-4" /> Upload brief / deck (placeholder)
                </div>
              </div>
              <div className="sm:col-span-2">
                <Label>Project / Partnership Goal</Label>
                <Input value={form.project_goal} onChange={(e) => set('project_goal', e.target.value)} placeholder="One-line goal" className="mt-2" />
              </div>
              <div className="sm:col-span-2">
                <Label>Message</Label>
                <Textarea rows={5} value={form.message} onChange={(e) => set('message', e.target.value)} placeholder="Share context, scope, or a brief." className="mt-2" />
              </div>
              <div className="sm:col-span-2 flex items-center justify-between gap-4 pt-2 flex-wrap">
                ...
                <Button type="submit" variant="gold" size="lg">Submit Proposal Request <ArrowUpRight className="h-4 w-4 ml-1" /></Button>
              </div>
            </form>
          </div>
          ...
          <aside className="lg:col-span-5 space-y-6">
            <div className="p-8 border border-border rounded-sm bg-card">
              ...
              <div className="mt-6 space-y-3">
                {documents.map((d) => (
                  <div key={d.title} className="flex items-center gap-4 p-4 border border-border rounded-sm hover:border-accent transition-colors">
                    ...
                    <Button variant="gold-outline" size="sm" onClick={() => toast('Upload PDF from admin', { description: 'Admin can attach a downloadable PDF for this document.' })}>
            ...
            <div className="p-8 border border-accent/40 bg-accent/[0.06] rounded-sm">
              ...
              <div className="mt-6 space-y-3">
                ...
                <Button variant="outline" className="w-full" onClick={() => toast('WhatsApp link placeholder', { description: 'Admin can connect the WhatsApp Business number.' })}>
