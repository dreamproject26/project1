...
import { getVentures, resolveMedia } from '@/lib/api';
import { heroImage } from '@/data/mockData';

const statuses = ['All', 'Active', 'Building', 'Upcoming'];

const Ventures = () => {
  const { data: ventures, loading } = useFetch(getVentures, []);
  ...
  }, [status, q, category, ventures]);

  const cats = useMemo(() => ['All', ...Array.from(new Set((ventures || []).map(v => v.category)))], [ventures]);
    ...
    <div>
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-24 bg-primary text-primary-foreground overflow-hidden">
        ...
        <div className="relative container-executive">
          ...
          <h1 className="mt-6 font-display font-semibold text-4xl sm:text-5xl lg:text-6xl leading-[1.05] max-w-4xl">The NN Venture portfolio.</h1>
          <p className="mt-6 text-lg text-primary-foreground/75 max-w-2xl">Diversified initiatives under founder leadership. Each venture is coordinated as part of a single, disciplined portfolio.</p>
        </div>
      </section>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {statuses.map(s => (
              <button key={s} onClick={() => setStatus(s)}
                className={`px-4 py-2 text-xs tracking-wider uppercase rounded-sm border transition-colors ${status === s ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:border-accent hover:text-foreground'}`}>{s}</button>
            ))}
          </div>
          <div className="flex items-center gap-2 flex-wrap md:ml-4">
            {cats.slice(0, 5).map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-3 py-1.5 text-[11px] tracking-wider rounded-full border transition-colors ${category === c ? 'bg-accent text-accent-foreground border-accent' : 'border-border text-muted-foreground hover:border-accent hover:text-foreground'}`}>{c}</button>
            ))}
          </div>
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container-executive">
          ...
          {!loading && filtered.length === 0 && (<div className="text-center py-20 text-muted-foreground">No ventures match your filters.</div>)}
        </div>
      </section>
    </div>
  );
