...
import { gallery, secondaryHero } from '@/data/mockData';
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-24 bg-primary text-primary-foreground overflow-hidden">
        ...
        <div className="relative container-executive">
          ...
          <h1 className="mt-6 font-display font-semibold text-4xl sm:text-5xl lg:text-6xl leading-[1.05] max-w-4xl">
            Founder photography, ventures, and moments.
          </h1>
        </div>
      </section>
        ...
        <div className="container-executive flex items-center gap-2 flex-wrap">
          {cats.map(c => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`px-4 py-2 text-xs tracking-wider uppercase rounded-sm border transition-colors ${cat === c ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:border-accent hover:text-foreground'}`}
            >{c}</button>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container-executive">
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 [column-fill:_balance]">
            {items.map((g, i) => (
              <figure key={i} className="mb-6 break-inside-avoid group relative overflow-hidden rounded-sm border border-border bg-card">
                <img src={g.image} alt={g.title} className="w-full h-auto object-cover group-hover:scale-105 transition-transform duration-700" />
